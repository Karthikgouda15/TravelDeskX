...
import {
  Search, SlidersHorizontal, ArrowRight, Plane, Clock, ShieldCheck,
  ArrowRightLeft, TrendingDown, TrendingUp, Users, Zap, Info, ChevronRight,
  CalendarDays, UserRound, ChevronDown, X, Wifi, BriefcaseBusiness, AlertTriangle,
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
const fareIntelligence = (flightDate) => {
...
// ─── Flight Card ─────────────────────────────────────────────────────────────
const FlightCard = React.memo(({ flight, index, isRecommendation, onSelect, isExpanded, onToggle }) => {
    ...
    >
      ...
      `}>
        {isRecommendation
          ...
          : <><ShieldCheck size={10} className="text-white" /><span className="text-[9px] font-black uppercase tracking-tighter text-white">TravelDesk Assured</span></>
        }
      </div>
      ...
      <div className="flex flex-col gap-3 relative z-10 pt-2">
        ...
        <div className="pt-3 border-t border-white/5 flex items-center justify-between flex-wrap gap-2 px-6 pb-2">
          ...
          <div className="flex items-center gap-4 text-[9px] font-black uppercase tracking-widest text-textMuted">
            ...
            <span className="flex items-center gap-1.5"><ShieldCheck size={10} className="text-white/40" />₹0 Cancellation</span>
